				...
				<table class="ta">
					...
					<c:forEach items="${sessionScope.hoslist }" var="hospital"
						...
						<tr bgColor="${status.index%2==0?'#ffffff':'#dde5f2' }">
							...
							<td>${hospital.memo }</td>
							<td><img class="img"
								src="${path }/images/icon_edit_blue.png" alt="编辑"
				...
				<center>
					<div id="pageDir">
						...
						--> <c:forEach var="i" begin="1" step="1" end="${lastIndex }">
									<a href="HospitalAction!hospitalList?index=${i } "
										target="mainFrame"><c:out value="${i }" /> </a>
								</c:forEach> <a href="HospitalAction?index=${lastIndex}" target="mainFrame">尾页</a>
							</li>
							<li style="margin-left:40px;"><s:form
									action="HospitalAction!hospitalList" theme="simple"
									target="mainFrame">
