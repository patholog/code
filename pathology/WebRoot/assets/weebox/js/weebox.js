...
(function ($) {
    ...
    var weebox = function (content, options) {
        ...
        this.initContent = function (content) {
            ...
            if (self.options.type == 'suggest') {//例外处理
                ...
                if (self.options.contentType == "selector") {
                    ...
                    html += ('\n</style>');
                    html += ('<iframe id="dialogIframe" class="dialogIframe" src="' + self._content + '" width="100%" height="100%" frameborder="0"></iframe>');
                    self.setContent(html);
                    self.onopen();
                    self.show();
                } else {
        ...
        this.setOncondclose = function (fn) {
            self.options.oncondclose = fn;
        }
        this.setOncondclose = function (fn) {
            self.options.oncondclose = fn;
        }
        ...
        this.next = function (opt) {
            ...
            opt.oncondclose = opt.oncondclose || null;
            ...
            if (typeof (opt.oncondclose) == "function") self.setOncondclose(opt.oncondclose);
            if (typeof (opt.oncancel) == "function") self.setOncancel(opt.oncancel);
            self.show();
        }
        //关闭弹窗
        this.close = function (n) {
            if (!n&&typeof (self.options.oncondclose) == "function") {
                if (!self.options.oncondclose(self)) { 
                    return;
                }
            }
